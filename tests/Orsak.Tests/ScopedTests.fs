...
type TestProvider() =
    ...
    interface IClockProvider with
        member this.Clock = { new IClock with member this.UtcNow() = System.DateTimeOffset.UtcNow }

    interface TransactionScopeProvider<DbTransactional> with
        member this.Scope() : ValueTask<DbTransactional> =
            vtask {
...
type Scoped() =
    ...
    let run e = Effect.runOrFail<TestProvider, _, string> (TestProvider()) e
