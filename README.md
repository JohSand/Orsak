...
<ItemGroup>
    ...
    <PackageReference Include="Myriad.Sdk" Version="1.1.0" />
    <!-- Required, see below -->
    <PackageDownload Include="Myriad" Version="[1.1.0]" />
</ItemGroup>
```
...
`MSB3954: Failed to compute hash for file '...\myriad\1.1.0\tools\net8.0\any\Myriad.dll'`.
...
instead of writing a separate file. The code is then generated without `namespace` or `open` declarations, so it continues the
file's last module or namespace. Myriad 1.1 and later tell generators when a file is inline, and only rewrite it when the generated
code changes, so edits elsewhere in the file leave it alone:
```fsharp
module rec MyTests
...
- The attributed types must be in the last namespace or module of the file, where the code is appended.
- With Myriad 1.0, which does not tell generators about inline generation, set `Inline = true` on the attributes instead,
  e.g. `[<GenEnvironment(Inline = true)>]`. From Myriad 1.1 the project's own setting takes precedence over it.
...
Myriad 1.x requires FSharp.Core 9 or later in any project that loads `Myriad.Core` directly.
